...
function file_get_dom($file, $return_root = true, $use_include_path = false, $context = null, $gzip = false) {
    if (version_compare(PHP_VERSION, '5.0.0', '>=')) {
        if ($gzip) {
            $f = gzdecode(file_get_contents($file, $use_include_path, $context));
        } else {
        ...
        if ($gzip) {
            $f = gzdecode(file_get_contents($file, $use_include_path));
        } else {
